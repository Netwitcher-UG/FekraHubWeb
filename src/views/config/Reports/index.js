...
export default function SchoolReports (){
  ...
  const {
    ...
    defaultValues: {
      ...
      city: '',
      job: '',
      StudentsReportsKeys: [{ EmailServer: '' }],
      graduation: '',
      birthday: null,
      gender: '',
      zipCode: ''
...
<Box sx={{margin:'40px'}}>
  ...
  <Box key={field.id} sx={{display:'flex',width:'100%',flexDirection:'row',margin:'24px'}}>
    <Controller
      name={`StudentsReportsKeys.${index}.EmailServer`}  // Ensure the correct index is used
      control={control}
      render={({ field }) => (
        <CustomTextField
          {...field}
          fullWidth
          label={`Students Reports Keys ${index + 1}`}
          placeholder='Please enter Students Reports Keys'
          error={!!errors.StudentsReportsKeys?.[index]?.EmailServer}
          helperText={errors.StudentsReportsKeys?.[index]?.EmailServer?.message}
        />
      )}
    />
    <Button sx={{marginX:'12px'}} variant='text' color='error'  onClick={() => remove(index)}>
      Remove
    </Button>
  </Box>
))}

<Button sx={{margin:'24px'}} variant='outlined'  onClick={() => append({ EmailServer: '' })}>
  Add New Key
</Button>
                    </Grid>
