...
import { updateProfileInfo } from 'src/store/apps/account'
...
const EditProfileInfoDialog = ({ open, setOpen, profileData }) => {
  ...
  const onSubmit = async data => {
    const response = await dispatch(updateProfileInfo(data))

    if (response?.payload?.status == 400) toast.error(response?.payload?.data)
    else if (response?.payload?.status == 200) {
      toast.success(<Translations text={'Your info updated successfully '} />, 1000)
