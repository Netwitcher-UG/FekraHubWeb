...
const AboutOverview = props => {
  const { about, role } = props
  const ability = useContext(AbilityContext)

  // Determine if this is a Secretariat role - use role from props or fallback to about object
  const userRole = role || about?.roles
          ...
          <CardContent sx={{ p: 4 }}>
            <Box sx={{ mb: 4, display: 'flex', alignItems: 'center', gap: 2 }}>
              ...
              <Box>
                ...
                >
                  <Translations text={isSecretariat ? 'Secretary Info' : 'Teacher Info'} />
                </Typography>
