...
import { editEmployee } from 'src/store/apps/users'
...
const EditEmployeeDialog = ({ open, setOpen, profileData, paramsQuery }) => {
  ...
  const onSubmit = async data => {
    ...
    if (response?.payload?.status == 400) toast.error(response?.payload?.data)
      ...
      handleClose()
      toast.success(<Translations text={'User info updated successfully '} />, 1000)
  ...
  const handleChageUserState = async status => {
    ...
    if (response?.payload?.status == 400) toast.error(response?.payload?.data)
      ...
      handleCloseDialog()
      handleClose()
      toast.success(<Translations text={`User ${status ? 'activated' : 'deactivated'}  successfully`} />, 1000)
    } else toast.error(response?.payload?.data || 'something went wrong try again !')
  }
  return (
    <Dialog
    ...
    >
      ...
      <Grid item xs={12}>
        <Typography variant='body2' sx={{ fontWeight: 600, m: 4 }}>
          2. <Translations text='User Details' />
        </Typography>
      </Grid>
